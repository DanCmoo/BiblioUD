...
public:
    ...
    Autor(const std::string& nom, const std::string& ape, bool sx, const std::string& fNac,
          const std::string& cNac, const std::string& pNac, const std::string& cRes,
          const std::string& form, int anioInicio, int anioPrimera)
        : idAutor(++countId), nombre(nom), apellido(ape), sexo(sx), fechaNacimiento(fNac),
    ...
    Autor(const std::string& id,const std::string& nom, const std::string& ape, bool sx, const std::string& fNac,
          const std::string& cNac, const std::string& pNac, const std::string& cRes,
          const std::string& form, int anioInicio, int anioPrimera)
        : idAutor(std::stoi(id)), nombre(nom), apellido(ape), sexo(sx), fechaNacimiento(fNac),
          ciudadNacimiento(cNac), paisNacimiento(pNac), ciudadResidencia(cRes),
          formacionBase(form), anioInicioLiteratura(anioInicio), anioPrimeraObra(anioPrimera) {}

    int getAnioNacimiento() const {
        if (fechaNacimiento.size() >= 4) {
            return std::stoi(fechaNacimiento.substr(0,4));
        }
    ...
    void  mostrar() const{
        std::cout << "ID: " << idAutor << ", Nombre: " << nombre << " " << apellido
             << ", Formacion: " << formacionBase << std::endl;
    }
};
